...
// And so this will actually get that data from local storage. if there is no session.session, which is actually the name, then we just return null here. So then there is really no current user. But if there is, then we can get that user again from Supabase. And once again, you might think that
